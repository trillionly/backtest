...
function cardMarkup(result) {
  return `
    <article class="result-card">
      <div>
        <p class="section-label">Saved result</p>
      ...
      <div class="card-footer">
        ...
        <div>
          <a class="button-link secondary" href="compare.html?files=${encodeURIComponent(result.file)}">Compare</a>
          <a class="button-link" href="strategy.html?file=${encodeURIComponent(result.file)}">Open detail</a>
        </div>
      </div>
    </article>
  `;
...
async function renderIndexPage() {
  try {
    ...
    document.getElementById("result-count").textContent = `${results.length} saved result file${results.length === 1 ? "" : "s"}`;
    if (results.length === 0) {
      showState("index-state", "No saved backtest results were found yet. Run `scripts/run_backtest.py` to generate a result JSON.");
      return;
    }
    ...
    document.getElementById("result-count").textContent = "Load failed";
    showState("index-state", "Could not load the result index. Open the site through a local server or GitHub Pages so the browser can fetch JSON files.");
  }
}
...
async function renderStrategyPage() {
  ...
  if (!file) {
    showState("strategy-state", "No result file was selected. Open a detail page from the results list.");
    return;
  }
  ...
  try {
    ...
    document.getElementById("strategy-subtitle").textContent = result.description || formatPeriod(result.period);

    document.getElementById("strategy-metrics").innerHTML = [
      metricCard("Final value", formatCurrency(result.summary?.final_value)),
      metricCard("Total return", formatPercent(result.summary?.total_return), Number(result.summary?.total_return) < 0 ? "negative-text" : "positive-text"),
    ...
    annualBody.innerHTML = annualReturns.length
      ? annualReturns
          ...
          .join("")
      : '<tr><td colspan="2">No annual return data found.</td></tr>';
    ...
    tradeBody.innerHTML = trades.length
      ? trades
          ...
          .join("")
      : '<tr><td colspan="7">No trades were recorded in this result file.</td></tr>';
    ...
    showState("strategy-state", "Could not load the selected result file. Check that the JSON exists and that you are serving the repository through HTTP.");
  }
}
...
async function renderCompareResults(selectedFiles) {
  ...
  if (selectedFiles.length < 2) {
    ...
    showState("compare-state", "Select at least two result files to build a comparison view.");
    return;
  }
...
async function renderComparePage() {
  try {
    const results = await loadResultsIndex();
    if (results.length === 0) {
      showState("compare-state", "No saved result files are available yet. Generate backtest results before using compare mode.");
      return;
    }
    ...
    showState("compare-state", "Could not load the result index. Open the site through a local server or GitHub Pages so JSON fetch requests can succeed.");
  }
}
